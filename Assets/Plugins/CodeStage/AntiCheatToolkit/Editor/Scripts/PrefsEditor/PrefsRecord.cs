namespace CodeStage.AntiCheat.EditorCode
{
	...
	internal class PrefsRecord
	{
		...
		{
			if (!newRecord && !ValidateCurrentData())
			{
				if (!EditorUtility.DisplayDialog("Stale Data Detected", 
					"The value for '" + key + "' has been modified externally.\n" +
					"Saving now may overwrite those changes.\n\n" +
					"Refresh the Prefs Editor to see current values.", "Save Anyway", "Cancel"))
				{
					return false;
				}
			}
		...
		{
			// Create a temporary record to read current PlayerPrefs value
			var currentRecord = new PrefsRecord(savedKey);
			
			// Compare current data with our cached data
			if (currentRecord.prefType != prefType) return false;
			if (currentRecord.Obscured != Obscured) return false;
			
			switch (prefType)
			{
				case PrefsType.String:
					return currentRecord.stringValue == stringValue;
				case PrefsType.Int:
					return currentRecord.intValue == intValue;
				case PrefsType.Float:
					return Math.Abs(currentRecord.floatValue - floatValue) < 0.0000001f;
				default:
